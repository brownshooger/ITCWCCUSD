...
Function Get-ComputerTab($computername) {
    $computerDT = Get-ADComputer -Filter "name -like '*$computername*'"   -Properties * | Select-Object Name, @{N = 'Computer OU'; E = { $_.CanonicalName } }, IPv4Address, OperatingSystem, whenCreated
    Return $computerDT
}
